...
{
    public class ParallelHashSetAStarRouteFinder : RouteFinder
    {
        public int ObjectClearance { get; set; }

        public ParallelHashSetAStarRouteFinder()
        {
        ...
        public override Route FindPath(PointF startPoint, PointF endPoint, Field field, IPositionedObject movingObject)
        {
            ...
            while (openSet.Any())
            {
                ...
                var cameFromLock = new ReaderWriterLockSlim();

                Parallel.ForEach(neighbourIndexes, i =>
                {
                    var neighbour = grid[i];

                    if (closedSet.Contains(neighbour))
                        return; // Already checked this square. Skip it
                    ...
                    openSetLock.ExitUpgradeableReadLock();

                    if (tentativeIsBetter)
                    {
                        cameFromLock.EnterWriteLock();
